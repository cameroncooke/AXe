...
function buildInstallSection(version) {
  ...
  return [
    ...
    '### Manual',
    '',
    `Download \`AXe-macOS-${normalizedVersion}.tar.gz\` from the assets below, extract, and place the \`axe\` binary on your \`PATH\`.`,
  ].join('\n');
}

function buildReleaseBody(version, changelogSection) {
  const normalizedVersion = normalizeVersion(version);
  const installSection = buildInstallSection(normalizedVersion);
  return [`## What's Changed`, '', changelogSection, '', installSection, ''].join('\n');
}

async function main() {
  try {
    const { changelog, out, version } = parseArgs(process.argv.slice(2));
